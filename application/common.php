...
function status($status){
	if($status == 1){
		return $str = "<span class='label label-success radius'>正常</span>";
	}elseif($status == 0){
		return $str = "<span class='label label-danger radius'>待审</span>";
	}elseif($status==-1){
		return $str = "<span class='label label-success radius'>删除</span>";
	}	
}
/* $type0 get 1 post */
...
class Pin{
	...
	//第二个参数随意设置则为utf-8编码
	//$pin = new Pin();
	//echo $pin->Pinyin(' 长 城 好 长','UTF8'); //输出 chang cheng hao chang
}

function wordsToPin($words){
	$pin = new Pin();
	return $pin->Pinyin($words,'UTF8');
}
